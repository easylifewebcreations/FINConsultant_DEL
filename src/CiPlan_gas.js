function AddDataRiderContract(_formObj, _sheetInfo, _folderID) {
  // Set result variable
  let result = EasyLifeLibrary.ResultInit(false)
  // Spreadsheet
  ...
  const existCriteria = EasyLifeLibrary.ExistCriteriaOR(spreadsheetID, sheetName, criteriaObj, keyHeader)
  if (!existCriteria) {
    // ******************** New item ********************
    // Get next data-id
    const headers = GetHeaders(sheetName)
    const newDataID = EasyLifeLibrary.GetNextIDWithFormat(spreadsheetID, sheetName, headers, keyHeader, RIDERCONTRACT_ID_SETTING)
    // Get & upload file
    ...
    if (dataForm.brochure.length > 0) {
      ...
      imageID = EasyLifeLibrary.UploadFile(_folderID, dataForm.brochure, newDataID)
      urls = EasyLifeLibrary.GetImageUrl(imageID)
      lh5 = urls.lh5
    } else {
...
function EditDataRiderContract(_formObj, _sheetInfo, _folderID) {
  ...
  const existCriteriaOR = EasyLifeLibrary.ExistCriteriaOR(spreadsheetID, sheetName, criteriaObj, keyHeader)
  // let existItem = {
  ...
  if (!existCriteria) {
    ...
    // ]
    // const existItem = EasyLifeLibrary.ReadItem(spreadsheetID, sheetName, criteriaObj)
    ...
    const readItem = EasyLifeLibrary.ReadItem(spreadsheetID, sheetName, criteriaOldItemObj)
    ...
    if (dataForm.brochure.length > 0) {
      // ******************** File uploaded ********************
      // Delete file
      const deleteFile = EasyLifeLibrary.DeleteFile(imageID)
      // Upload & get file info
      imageID = EasyLifeLibrary.UploadFile(_folderID, dataForm.brochure, dataForm.dataID)
      const urls = EasyLifeLibrary.GetImageUrl(imageID)
      lh5 = urls.lh5
    }
